...
bool SILowerSGPRSpills::runOnMachineFunction(MachineFunction &MF) {
  ...
  if (HasSGPRSpillToVGPR) {
    ...
    // Initialize dynamic regclasses before lowering the spills and then turn
    // WWM_VGPR_32 classes visible so that wwm virtual registers can be
    // created.
    MRI.initializeDynRegClassHiddenInfo();
    MRI.changeRegClassVisibility(AMDGPU::WWM_VGPR_32RegClassID);
